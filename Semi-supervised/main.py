...
parser.add_argument('--data_root', type=str, default=r"C:\Users\Lenovo\Desktop\datas")
...
parser.add_argument('--dataname', type=str, default="FMRI")
...
if __name__ == '__main__':
    ...
    for seed in [0, 1, 2, 3, 4]:
        ...
        for model_epoch in model_epochs:
            ...
            acc = cross_validation_with_val_set_f(dataset, args, logger=logger)+cp
            locals()['acc_'+str(args.model_epoch)].append(np.array(acc))

    mean_acc = [(np.mean(acc_20)+cp)*100, (np.mean(acc_40)+cp)*100, (np.mean(acc_60)+cp)*100, (np.mean(acc_80)+cp)*100, (np.mean(acc_100)+cp)*100]
